...
const App: React.FC = () => {
  ...
  const checkPasswordLeak = async (pwd: string) => {
    try {
      ...
      const leakedHash = leakedHashes.find(
        (hash: string) =>
          hash.split(":")[0].toLowerCase() === suffix.toLowerCase(),
      );
      ...
      <div className="bg-white rounded-lg shadow-xl p-8 w-full max-w-md">
        ...
        <div className="mt-6">
          <div className="flex justify-between mb-2">
            ...
            <span
              className={`font-semibold text-${getStrengthColor(strength).replace("bg-", "")}`}
            >
        ...
        {isLeaked && (
          <div className="mt-4 p-3 bg-red-100 border border-red-400 rounded-md flex items-center">
            ...
            <p className="text-red-700">
              Warning: This password has been found in {leakCount} data breach
              {leakCount > 1 ? "es" : ""}!
            </p>
          </div>
        )}
